...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/logs":
            ...
            log_path = os.path.join(self.server.outdir, f"trae-debug-log-{self.server.session_id}.ndjson")
            logs = []
...
def main():
    ...
    parser.add_argument("--remote", action="store_true", help="Listen on 0.0.0.0 and show local IP")
    ...
    while True:
        try:
            ...
            with TCPServer((args.host, current_port), Handler) as httpd:
                ...
                print(f"[Debug Server] Listening on http://{display_host}:{current_port}")
            ...
            if e.errno in (98, 48, 10048):
                print(f"[Debug Server] Port {current_port} occupied, trying {current_port + 1}...")
                current_port += 1
                if current_port > args.port + 10:
                    print("[Error] Failed to find available port after 10 retries")
                    exit(1)
                continue
