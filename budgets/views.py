...
def _call_openai_analysis(report_text: str, user_followup: Optional[str] = None) -> LLMResult:
    ...
    if user_followup:
        ...
        prompt_sistema = (
            ...
            "- OBRIGATORIO: Informe o VALOR TOTAL em R$ (reais) de RECEITA PARADA / ATRASADA (campo 'VALOR TOTAL ATRASADO' do relatorio).\n"
            "- OBRIGATORIO: Informe tambem o impacto ponderado em R$·dias.\n\n"
            "# (2) PRINCIPAIS GARGALOS IDENTIFICADOS\n"
            "Liste top 3 gargalos, com evidencia (dados do relatorio). Sempre que possivel, associe um VALOR R$ ao gargalo (quanto dinheiro esta parado em cada causa).\n\n"
            "# (3) ANALISE DOS ATRASOS POR FAIXA\n"
            "1-2 dias, 3-5 dias, +5 dias: por que cada grupo esta atrasando? SEMPRE informe o VALOR R$ de cada faixa (campos 'DISTRIBUICAO FINANCEIRA POR FAIXA' do relatorio).\n\n"
            "# (4) IMPACTO FINANCEIRO E OPERACIONAL\n"
            "- OBRIGATORIO: DESTACAR o VALOR EXATO em R$ que a oficina DEIXOU DE RECEBER (ou esta com receita imobilizada) por causa dos atrasos. "
            "Use o numero do campo 'VALOR TOTAL ATRASADO (receita imobilizada/parada)'.\n"
            "- Composicao: quanto R$ em MO de obra, quanto R$ em PECAS, quanto R$ em SERVICOS.\n"
            ...
            "Cada acao ESPECIFICA para a oficina. Sempre que possivel, associe uma ECONOMIA / RETORNO em R$ para a acao.\n\n"
            "# (6) RECOMENDACOES INDIVIDUAIS POR ORCAMENTO CRITICO\n"
            "Selecione os 3-5 piores casos (+5d ou atraso + grande estouro) e 1 sugestao ESPECIFICA por caso.\n"
            "Inclua para cada caso: OS #, valor do reparo em R$, dias de atraso, e acao.\n\n"
            "REGRAS IMPORTANTISSIMAS (CUMPRA SEMPRE):\n"
            "1) NUNCA invente valores monetarios. Use APENAS os valores em R$ (BRL, REAL BRASILEIRO) fornecidos no relatorio bruto.\n"
            "2) SEMPRE responda em R$ (reais), usando formato R$ 1.234,56. NUNCA converta para USD. NUNCA use cifroes sem R$.\n"
            "3) Se o usuario perguntar 'quanto deixei de receber' ou 'valor do atraso' ou 'receita parada': responda DIRETAMENTE com o VALOR TOTAL ATRASADO do relatorio. Depois complemente com os detalhes por faixa.\n"
            "4) Use negrito e bullets. Max 2000 palavras total.\n"
            "5) Seja ACIONAVEL: toda recomendacao deve ter uma acao concreta que o gerente consegue executar no mesmo dia."
        )
        messages = [
